...
def check_sphinx_build(pkg_name, verbose=True, regenerate=True):
    ...
    doc_src = os.path.abspath(os.path.join(
        os.path.dirname(pkg.__file__), '../doc'))
    ...
    if not os.path.exists(doc_build) or regenerate:
        ...
        os.chdir(doc_src)
        cmd = ('sphinx-build -b html -d _build/doctrees '
...
def doc_to_docsets(pkg_name='fluiddyn', verbose=False, archive=False):
    ...
    if archive:
        doc_archive = os.path.abspath(
            os.path.join(doc_dest, docset_name))
...
def main():
    ...
    parser.add_argument(
        'package', help='Name of a FluidDyn package', type=str)
