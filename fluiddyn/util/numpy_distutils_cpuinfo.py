...
Taken from numpy/distutils/cpuinfo.py (deprecation of numpy.distutils)
...
class LinuxCPUInfo(CPUInfoBase):
    ...
    def _is_PentiumII(self):
        return (
            re.match(r".*?Pentium.*?II\b", self.info[0]["model name"]) is not None
        )
    ...
    def _is_PentiumM(self):
        return (
            re.match(r".*?Pentium.*?M\b", self.info[0]["model name"]) is not None
        )
