...
def dump_nc_object(obj, level=0, print_variables=False):
    ...
    if len(obj.variables) > 0:
        ...
        for value in obj.variables.values():
            print(indentp1 + '- variable {} (dimensions {}, dtype {})'.format(
    ...
    if len(obj.groups) > 0:
        ...
        for group in obj.groups.values():
            print()
            dump_nc_object(group, level=level+1,
...
def dump_h5_object(obj, level=0, print_variables=False):
    ...
    if len(variables) > 0:
        ...
        for value in variables:
            print(indentp1 + '- variable {} (dtype {})'.format(
    ...
    if len(groups) > 0:
        ...
        for group in groups:
            print()
            dump_h5_object(group, level=level+1,
                           print_variables=print_variables)
...
def main():
    ...
    parser.add_argument(
        'file', help='str indicating which file has to be dump.',
        type=str)
    ...
    if os.path.isfile(args.file):
        ...
        if len(paths) == 0:
            raise ValueError(
                'No file found from the input string:\n' + args.file)
        for path in paths:
